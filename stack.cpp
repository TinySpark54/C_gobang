...
void shape_push(ChangeMode mode, Point position, Shape shape, char direction)
{
    ...
    *ptr_change_stack++ = log;
    ...
    char total_length = total_pieces+shape.length1?1:0;
    ...
    if (shape.length1!=0)
    {
        ...
        if (shape.isblocked_begin)
        {
            ...
            {
                heat_push({back_X,back_Y},(shape.length0 >2?value_free[shape.length0]:value_blocked[total_pieces<4?total_pieces:3])*add);
                heat_push({broke_X,broke_Y},value_blocked[total_pieces]*add);
                score_push((shape.length0 >3?value_free[shape.length0-1]:value_blocked[total_pieces<4?total_pieces:3])*add*player);
            }
        }
        else
        {
            if (shape.isblocked_end)
            {
                heat_push({front_X,front_Y},(shape.length0 >2?value_free[shape.length0]:value_blocked[total_pieces<4?total_pieces:3])*add);
                heat_push({broke_X,broke_Y},value_blocked[total_pieces]*add);
                score_push((shape.length0 >2?value_free[shape.length0-1]:value_blocked[total_pieces<4?total_pieces:3])*add*player);
            }
            else
            {
                heat_push({back_X,back_Y},(total_pieces<3?value_free[total_pieces]:value_blocked[3])*add);
                heat_push({front_X,front_Y},(total_pieces<3?value_free[total_pieces]:value_blocked[3])*add);
                heat_push({broke_X,broke_Y},value_blocked[total_pieces]*add);
                score_push((value_blocked[total_pieces<4?total_pieces:3])*player*add);
            }
        }
    }
    else if (!shape.isblocked_end)//连
    {
        if (shape.isblocked_begin)
        {
